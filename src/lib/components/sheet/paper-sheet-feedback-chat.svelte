<script lang="ts">
	...
	const attachmentActionsEnabled = $derived(
		allowAttachments && typeof onAttachFiles === 'function'
	);
	...
	const composerInputClass = $derived(
		runtimeLocked
			? 'paper-sheet-note__input paper-sheet-note__input--locked'
			: 'paper-sheet-note__input'
	);
	...
	function handleComposerInput(detail: { value: string }): void {
		onDraftChange(detail.value);
	}

	function handleComposerSubmit(detail: { value: string }): void {
		if (composerDisabled) {
			return;
		}
		onReply(detail.value);
	}
</script>
...
{#if processing}
	<div class="paper-sheet-note__processing" role="status" aria-live="polite">connecting…</div>
{/if}
...
{#if showComposer}
	<div
		class={`paper-sheet-note__composer ${runtimeLocked ? 'is-runtime-locked' : ''} ${resolvedFollowUpMode ? 'is-follow-up' : ''}`}
	>
		<input
		...
		{#if draftAttachments.length > 0}
			<div class="paper-sheet-note__attachments is-draft">
				{#each draftAttachments as attachment (attachment.localId)}
					...
					<div
						class={`paper-sheet-note__attachment-card ${isImage && attachment.previewUrl ? 'is-image' : 'is-file'}`}
					>
		...
		<ReplyComposer
			...
			submitMode="enter"
			maxLines={5}
			showAttach={attachmentActionsEnabled}
			showMic={false}
			showTakePhoto={attachmentActionsEnabled && allowTakePhoto}
			{submitReady}
			...
			onTakePhotoSelect={() => {
				if (!attachmentActionsEnabled || !allowTakePhoto) {
					return;
				}
