...
export type PaperSheetFillQuestion = z.infer<typeof PaperSheetFillQuestionSchema>;

export const PaperSheetMcqDisplayModeSchema = z.enum(['full_options', 'labels_only']);

export type PaperSheetMcqDisplayMode = z.infer<typeof PaperSheetMcqDisplayModeSchema>;
...
export const PaperSheetMcqQuestionSchema = z
	...
	.superRefine((question, ctx) => {
		...
		for (let index = 0; index < question.options.length; index += 1) {
			...
			if (question.displayMode === 'labels_only' && (option.label?.trim().length ?? 0) === 0) {
				ctx.addIssue({
					code: z.ZodIssueCode.custom,
...
export type PaperSheetLinesQuestion = z.infer<typeof PaperSheetLinesQuestionSchema>;
...
export type PaperSheetMatchQuestion = z.infer<typeof PaperSheetMatchQuestionSchema>;
...
export type PaperSheetSpellingQuestion = z.infer<typeof PaperSheetSpellingQuestionSchema>;
...
export type PaperSheetClozeQuestion = z.infer<typeof PaperSheetClozeQuestionSchema>;
...
export type PaperSheetAnswerBankOption = z.infer<typeof PaperSheetAnswerBankOptionSchema>;
	...
	.superRefine((question, ctx) => {
		if (question.segments.length !== question.blanks.length + 1) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				path: ['segments'],
				message: 'Answer-bank questions need exactly one more segment than blank.'
			});
		}
		...
		if (question.allowReuse !== true && question.options.length < question.blanks.length) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				path: ['options'],
				message: 'Answer-bank questions without reuse need at least as many options as blanks.'
			});
		}
			...
			for (let index = 0; index < question.options.length; index += 1) {
				...
				if ((option.label?.trim().length ?? 0) === 0) {
					ctx.addIssue({
						code: z.ZodIssueCode.custom,
						path: ['options', index, 'label'],
						message: 'Answer-bank questions in banked mode need source-faithful option labels.'
					});
				}
			}
		}
...
export type PaperSheetAnswerBankQuestion = z.infer<typeof PaperSheetAnswerBankQuestionSchema>;
...
export type PaperSheetFlowConnector = z.infer<typeof PaperSheetFlowConnectorSchema>;
...
export type PaperSheetFlowQuestion = z.infer<typeof PaperSheetFlowQuestionSchema>;
...
export type PaperSheetQuestionGroup = z.infer<typeof PaperSheetQuestionGroupSchema>;
...
export type PaperSheetQuestionEntry = z.infer<typeof PaperSheetQuestionEntrySchema>;
...
export const PaperSheetContentSectionSchema = z
	...
	.superRefine((section, ctx) => {
		const hasTheory = typeof section.theory === 'string' && section.theory.trim().length > 0;
		...
		if (!hasTheory && !hasInfoBox && questionCount === 0) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				path: ['questions'],
				message: 'Worksheet content sections need at least one question, theory block, or info box.'
			});
		}
	});

export type PaperSheetContentSection = z.infer<typeof PaperSheetContentSectionSchema>;
...
export type PaperSheetQuestionReviewStatus = z.infer<typeof PaperSheetQuestionReviewStatusSchema>;
...
export type PaperSheetQuestionReview = z.infer<typeof PaperSheetQuestionReviewSchema>;
...
export type PaperSheetFeedbackAttachment = z.infer<typeof PaperSheetFeedbackAttachmentSchema>;
...
export type PaperSheetFeedbackTurn = z.infer<typeof PaperSheetFeedbackTurnSchema>;
...
export type PaperSheetFeedbackThread = z.infer<typeof PaperSheetFeedbackThreadSchema>;
...
export type SparkGraderWorksheetReferences = z.infer<typeof SparkGraderWorksheetReferencesSchema>;
...
function normalizeHexColor(value: unknown, fallback: string): string {
	return typeof value === 'string' && /^#[0-9a-fA-F]{6}$/u.test(value) ? value : fallback;
}
...
function normalizeLegacyMcqOption(option: unknown, index: number): PaperSheetMcqOption | null {
	...
	const label =
		asTrimmedStringOrNull(record.label) ?? asTrimmedStringOrNull(record.id) ?? undefined;
...
function normalizeLegacyAnswerBankOption(
	...
	const text = asTrimmedStringOrNull(record.text) ?? asTrimmedStringOrNull(record.value) ?? null;
	if (!text) {
		return null;
	}

	const label =
		asTrimmedStringOrNull(record.label) ?? asTrimmedStringOrNull(record.optionLabel) ?? undefined;
...
function normalizeLegacyFlowQuestion(
	...
	const boxes = rawBoxes
		.map((entry, index) => {
			...
			const readonly = record.readonly === true || record.editable === false ? true : undefined;
			return {
				id,
	...
	const rows = [...groupedBoxes.entries()]
		...
		.map(([prefix, group]) => {
			...
			const forwardLabel = first && last ? arrowLabels.get(`${first.id}->${last.id}`) : undefined;
			...
			for (let index = 0; index < ordered.length; index += 1) {
				...
				const label =
					arrowLabels.get(`${box.id}->${next.id}`) ?? arrowLabels.get(`${next.id}->${box.id}`);
				if (!label) {
					continue;
				}
				items.push({
...
function normalizeLegacyQuestion(
	...
	const prompt =
		asTrimmedStringOrNull(record.prompt) ?? asTrimmedStringOrNull(record.promptMarkdown);
	...
	switch (type) {
		case 'mcq': {
			...
			const displayMode = record.displayMode === 'labels_only' ? 'labels_only' : 'full_options';
			...
			if (rawBlanks && rawBlanks.length >= 1 && rawBlanks.length <= 2) {
				...
				if (blanks.length === rawBlanks.length) {
					const structuredFill = PaperSheetFillQuestionSchema.safeParse({
						...
						blanks,
						after: rawAfter,
						...(rawConjunction !== undefined ? { conjunction: rawConjunction } : {})
					});
		...
		case 'answer_bank': {
			...
			const options = rawOptions
				...
				.filter((option): option is PaperSheetAnswerBankOption => option !== null);
			if (options.length !== rawOptions.length) {
				return null;
			}
			...
			if (rawSegments && rawBlanks) {
				...
				const segments = rawSegments.map((segment) => (typeof segment === 'string' ? segment : ''));
				const parsed = PaperSheetAnswerBankQuestionSchema.safeParse({
					...
					...(badgeLabel ? { badgeLabel } : {}),
					marks,
					displayMode: record.displayMode === 'banked' ? 'banked' : 'inline_labeled',
					segments,
					blanks,
					options,
		...
		case 'flow':
			...
			return normalizeLegacyFlowQuestion(record, id, displayNumber, badgeLabel, marks, prompt);
		default:
			return null;
	}
}
...
function normalizeLegacySection(value: unknown, index: number): PaperSheetSection | null {
	...
	const title = asTrimmedStringOrNull(record.label) ?? asTrimmedStringOrNull(record.title);
	if (!title) {
...
export function coerceSparkSolveSheetDraft(value: unknown): SparkSolveSheetDraft | null {
	...
	const references = SparkGraderWorksheetReferencesSchema.safeParse(record.references);
...
export type SparkSolveSheetAnswers = z.infer<typeof SparkSolveSheetAnswersSchema>;

function collectSheetQuestions(sheet: PaperSheetData): Map<string, PaperSheetQuestion> {
...
export const SparkGraderWorksheetReportSchema = z
	...
	.superRefine((report, ctx) => {
		...
		const totalMarks = [...questions.values()].reduce((sum, question) => sum + question.marks, 0);
		...
		for (const [key, answer] of Object.entries(report.answers)) {
			...
			if (question.type === 'mcq' || question.type === 'lines' || question.type === 'calc') {
			...
			if (question.type === 'flow') {
				for (const box of question.boxes) {
					const requiresAnswer = box.initialValue === undefined && box.readonly !== true;
...
export type SparkGraderWorksheetReport = z.infer<typeof SparkGraderWorksheetReportSchema>;
